...
export async function getServerSideProps( context ) {
    ...
    const breadcrumbs = [
        {
            label: 'Service List',
            link: '/admin/contents/dealership-services'
        },
        {
            label: `Edit Service: ${ res.data.name }`
        }
    ];
    // return props with fetched data
    return {
        props: {
            titlePage: `Edit Service: ${ res.data.name }`,
            providers,
            csrfToken,
            breadcrumbs,
            content: res.data
...
const EditPage = ( { content, csrfToken, titlePage = '', breadcrumbs } ) => {
    const { formRef, form, valid, handleChange, hanldeBlur } = useForm(
        {
            name: content.name, slug: content.slug, title: content.slug, Keyword: content.slug,
        },
                    ...
                    <Grid container>
                        ...
                        <Grid item xs={ 12 } md={ 5 }>
                            ...
                            <BlockItemForm icon="check-square" title="Status" marginY="40px" sx={ { mx: { xs: 0, md: 3 } } }>
                                ...
                                {/* action button */ }
                                <ActionButton href="/admin/contents/dealership-services" />
                            </BlockItemForm>
                            {/* Attribute page */ }
                            <BlockItemForm icon="paperclip" title="Attribute Page" marginY="40px" sx={ { mx: { xs: 0, md: 3 } } }>
                                <CartInputSelect label="Attribute page" options={ OPTIONS_SELECT } defaultOption="ĐỘ RỘNG" />
                            </BlockItemForm>
                        </Grid>
                    </Grid>
                </Box>
